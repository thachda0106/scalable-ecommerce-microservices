...
export class PayPalProvider implements IPaymentProvider {
  ...
  private async simulatePayPalCall(request: PaymentProviderRequest): Promise<PaymentProviderResult> {
  ...
  private async simulatePayPalRefund(transactionId: string): Promise<RefundResult> {
  ...
  private withTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
    return Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error(`PayPal request timed out after ${timeoutMs}ms`)), timeoutMs),
      ),
    ]);
  }
}
