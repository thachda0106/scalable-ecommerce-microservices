...
export class CheckoutSagaService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.consumer = this.kafka.consumer({
      groupId: 'order-service-checkout-saga',
    });
  }
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      'Saga Orchestrator connected and listening to orchestrate checkout',
    );
  }
  ...
  private async handleSagaEvent(event: any) {
    ...
    if (type === 'InventoryReserved') {
      ...
      this.logger.log(
        `Order ${orderId}: Inventory reserved. Awaiting payment.`,
      );
  ...
  private async updateOrderStatus(
    ...
    try {
      ...
      this.logger.error(
        `Saga update error for order ${orderId}: ${err.message}`,
      );
    } finally {
